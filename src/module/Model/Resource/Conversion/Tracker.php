...
class Mzax_Emarketing_Model_Resource_Conversion_Tracker extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    public function enable($trackerIds)
    {
        return $this->_getWriteAdapter()->update($this->getMainTable(), array(
            'is_active'     => 1,
            'is_aggregated' => 0
        ), array('is_active != 1', 'tracker_id IN(?)' => $trackerIds));
    }
    ...
    public function disable($trackerIds)
    {
        return $this->_getWriteAdapter()->update($this->getMainTable(), array(
            'is_active'     => 0,
            'is_aggregated' => 0
        ), array('is_default != 1', 'is_active != 0', 'tracker_id IN(?)' => $trackerIds));
    }
    ...
    public function flagAsAggregated($trackerIds)
    {
        return $this->_getWriteAdapter()->update($this->getMainTable(), array(
            'is_aggregated' => 1
        ), array('is_active = 1', 'is_aggregated != 1', 'tracker_id IN(?)' => $trackerIds));
    }
    ...
    public function massDelete($trackerIds)
    {
        return $this->_getWriteAdapter()->delete($this->getMainTable(), 
                array('is_default != 1', 'tracker_id IN(?)' => $trackerIds));
    }
