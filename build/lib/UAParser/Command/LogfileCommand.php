...
class UAParser_Command_LogfileCommand extends Symfony_Component_Console_Command_Command
{
    protected function configure()
    {
        $this
            ...
            ->addOption(
                'log-file',
                'f',
                Symfony_Component_Console_Input_InputOption::VALUE_REQUIRED,
            ...
            ->addOption(
                'include',
                'i',
                Symfony_Component_Console_Input_InputOption::VALUE_IS_ARRAY | Symfony_Component_Console_Input_InputOption::VALUE_REQUIRED,
    ...
    protected function execute(Symfony_Component_Console_Input_InputInterface $input, Symfony_Component_Console_Output_OutputInterface $output)
    {
        if (!$input->getOption('log-file') && !$input->getOption('log-dir')) {
            throw UAParser_Exception_InvalidArgumentException::oneOfCommandArguments('log-file', 'log-dir');
        }

        $parser = UAParser_Parser::create();
        $undefinedClients = array();
        /** @var $file Symfony_Component_Finder_SplFileInfo */
        foreach ($this->getFiles($input) as $file) {
            ...
            $reader = UAParser_Util_Logfile_AbstractReader::factory($firstLine);
            if (!$reader) {
            ...
            foreach ($lines as $line) {
                ...
                } catch (UAParser_Exception_ReaderException $e) {
    ...
    private function outputProgress(Symfony_Component_Console_Output_OutputInterface $output, $result, $count, $totalCount, $end = false)
    {
        if (($count % 70) === 0 || $end) {
    ...
    private function getResult(UAParser_Result_Client $client)
    {
    ...
    private function getFiles(Symfony_Component_Console_Input_InputInterface $input)
    {
        $finder = Symfony_Component_Finder_Finder::create();

        if ($input->getOption('log-file')) {
            $file = $input->getOption('log-file');
            $finder->append(Symfony_Component_Finder_Finder::create()->in(dirname($file))->name(basename($file)));
        }

        if ($input->getOption('log-dir')) {
            $dirFinder = Symfony_Component_Finder_Finder::create()
    ...
    private function getPath(Symfony_Component_Finder_SplFileInfo $file)
    {
